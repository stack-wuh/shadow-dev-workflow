...
description: >
  ...
  失败回环: 审查 ✗ → 回到执行修复。
  触发词: 提案 (优先级最高 → discuss); 新需求/propose; 需求讨论/explore/design; 开始执行/apply; 代码审查/review/verify; 归档/archive; 提交/commit/PR。
...
→ 下一步: [2/6] 需求讨论（full 模式）/ [3/6] 开始执行（ff 模式）
...
→ ✅ **工作流完成** — 关注 CI 结果和 code review 反馈
