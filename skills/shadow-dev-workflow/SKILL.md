...
变更命名: `YYYY-MM-DD-{P|B}-{kebab-case}`，如 `2026-07-05-P-add-comment-api`；`P` 表示新需求，`B` 表示新 bug。

---

## 🛡️ 防幻觉提示门禁

进入任何大流程或小流程前，必须先向用户打印对应流程提示，再执行读文件、写代码、运行命令、GitHub 操作或调用下游技能。该提示是防幻觉门禁，不得省略、合并或仅在内部思考中使用。
...
- 路由到子 Skill 后，先完整读取该子 Skill 的 `SKILL.md`，再打印即将执行的小流程提示。
- 若子 Skill 定义了编号步骤、检查项、门禁、失败回环或输出要求，必须在执行前用简短中文列出本次相关项。
- 若用户只给出短触发词（如 `提案`、`开始执行`、`代码审查`），仍必须打印当前流程提示；缺少关键输入时，在提示之后只问一个必要问题。
- 不得在流程提示打印前扫描活跃变更、修改文件、启动测试、提交代码或访问 GitHub。
- 中断恢复时，先打印“中断恢复”提示，再执行 `openspec list` 或读取上下文。
